...
// Decode a VIN via the free NHTSA API → "2005 Chevrolet Suburban 1500 5.3L"
async function decodeVin(vin) {
  const clean = (vin || "").trim().toUpperCase();
  if (clean.length !== 17) throw new Error("A VIN is 17 characters.");
  ...
  const parts = [
    r.ModelYear, r.Make, r.Model, r.Trim,
    r.DisplacementL ? `${Number(r.DisplacementL).toFixed(1)}L` : "",
  ].filter(Boolean);
  if (!r.Make && !r.Model) throw new Error("Couldn't read that VIN.");
  // Title-case make/model which come back ALL CAPS
  return parts
    .map((p) => /^[A-Z]+$/.test(p) ? p[0] + p.slice(1).toLowerCase() : p)
    .join(" ");
}
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        ...
        <VinField onDecoded={(text) => set({ vehicle: text })} />
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        ...
        <VinField onDecoded={(text) => set({ vehicles: d.vehicles ? `${d.vehicles}; ${text}` : text })} />
...
// VIN decode row: paste a VIN, tap Decode, it fills the target field.
function VinField({ onDecoded }) {
  ...
  const go = async () => {
    setBusy(true); setErr("");
    try {
      const text = await decodeVin(vin);
      onDecoded(text);
      setVin("");
    } catch (e) {
      setErr(e.message || "Couldn't decode VIN.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <label style={S.label}>Decode by VIN (optional)</label>
      <div style={{ display: "flex", gap: 8 }}>
        <input
          ...
          maxLength={17}
          onChange={(e) => { setVin(e.target.value); setErr(""); }}
          onKeyDown={(e) => e.key === "Enter" && go()}
        />
        <button style={S.ghostBtn} onClick={go} disabled={busy}>
          {busy ? "…" : "Decode"}
        </button>
      </div>
      {err && <div style={{ ...S.errText, fontSize: 12 }}>{err}</div>}
    </div>
  );
}
